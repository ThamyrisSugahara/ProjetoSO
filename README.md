# Trabalho de Sistemas Operacionais 2026.1 — MINIX

---

**Joahannes Bruno Dias da Costa**  
Departamento de Ciência da Computação  
UNIFESP - Universidade Federal de São Paulo

---

## Grupo 3

| RA      | Nome                             |
|---------|----------------------------------|
| 170327  | Gabriel Venerando Santos         |
| 168774  | Iago Samuel Ayres de Lima        |
| 150143  | Rian Silva Santos                |
| 170597  | Tales dos Santos Fernandes       |
| 169242  | Thamyris Nunes Sugahara          |
| 158555  | Luís Felipe Salgado Mitidieri    |

---
## Etapas
### Etapa 1 — Configurações e Alterações iniciais

1. Instalação e configuração do sistema operacional **MINIX 3.4.0rc6**.
2. Modificações em **banners do sistema** e recompilação do kernel.
3. Alterações na **listagem de comandos** e nova compilação do sistema.

### Etapa 2 — Modificações de implementação

1. Projeção e implementação de **três algoritmos de escalonamento de processos**.
2. Avaliação de desempenho dos algoritmos com diferentes cargas de processos para comparação.

### Etapa 3 — Documentação

1. Produção do **relatório técnico** descrevendo detalhadamente todas as modificações, seguindo o **template da Sociedade Brasileira de Computação (SBC)**. 

---


